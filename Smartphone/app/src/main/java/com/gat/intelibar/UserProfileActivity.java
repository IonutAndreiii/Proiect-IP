...
public class UserProfileActivity  extends AppCompatActivity {
    ...
    private static final byte[] deviceUID = "173289269318420".getBytes();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SetDataFromDB(new UserProfile(none, none, none, none, ""));

        Bluetooth.start_handler(context);
        connectToBluetoothDevice();
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction(Bluetooth.GOT_PROFILE_DATA);
    ...
    protected void onResume() {
        //chestii ce se intampla pe parcurs
        super.onResume();
        //profile.setBackgroundResource(R.drawable.gat);
    }

    private void SetDataFromDB(UserProfile user)
    {
...
/*        Bitmap image = BitmapFactory.decodeFile(file);
    ...
    private class BluetoothBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(action == Bluetooth.GOT_PROFILE_DATA) {
                ...
                Log.d(TAG, "Car number is: " + carNo);
                Log.d(TAG, "Schedule number is: " + schedule);
                UserProfile user = new UserProfile(name, division, carNo, schedule, hardcodedImage);
                SetDataFromDB(user);
            }
        }
    }
